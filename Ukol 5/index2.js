...
app.post("/todos", async (c) => {
  ...
  // Validace, že Todo není prázdné
  const title = form.get("title");
  if (!title || title.trim() === "") {
    throw new Error("Nesmíš vložit prázdné Todo.");
  }

  // Přečteme prioritu, nastavíme výchozí hodnotu "low", pokud není vybrána + validace priority z rozsahu
  const validPriorities = ["low", "medium", "high"];

  const priorityValue = form.get("priority") || "low";

  if (!validPriorities.includes(priorityValue)) {
    throw new Error(`Nesprávná hodnota proměnné priority: ${priorityValue}`);
  }
